...
websocket-connection_lost-desc = VoidReality 서버에 오류가 발생한 것 같습니다. 로그를 확인하고 프로그램을 재시작해 주세요
websocket-timedout = 서버에 연결할 수 없습니다
websocket-timedout-desc = VoidReality 서버에 오류가 발생했거나 연결 시간이 초과된 것 같습니다. 로그를 확인하고 프로그램을 재시작해 주세요
websocket-error-close = VoidReality 종료
...
tips-turn_on_tracker = 공식 VoidReality 트래커를 사용 중이신가요? 트래커를 <b><em>PC에 연결</em></b>하고 <b><em>전원을 키셔야</em></b> 해요.
...
tracker-settings-forget-description = VoidReality 서버에서 트래커를 제거하고 서버를 다시 시작할 때까지 자동으로 연결하지 않아요. 트래커의 설정은 지워지지 않아요.
...
settings-general-interface-connected_trackers_warning-description = 이 옵션은 트래커가 연결되어 있는 채로 VoidReality을 종료하려고 할 때 팝업을 표시합니다. 그렇게 하면 실수로 트래커를 끄지 않아 배터리가 방전되는 일을 예방할 수 있어요.
...
settings-general-interface-use_tray-description = VoidReality 서버를 닫지 않고 창만 닫을 수 있게 하여 사용 시 항상 GUI를 띄워 놓을 필요가 없게 해요.
settings-general-interface-use_tray-label = 작업 표시줄로 최소화
settings-general-interface-discord_presence = Discord에서 활동 공유
settings-general-interface-discord_presence-description = Discord 활동 상태에 VoidReality을 사용 중이라는 것과 사용 중인 트래커의 개수를 같이 표시합니다.
...
settings-osc-router-network-description =
    ...
    이들은 VoidReality 서버에서 사용되는 다른 포트와 동일해도 돼요.
...
settings-osc-vmc-description =
    VoidReality의 골격 데이터를 보내고 다른 앱에서 이 데이터를 수신하기 위해
...
onboarding-setup_warning =
    ...
    VoidReality을 처음 사용하는 경우에도 필요합니다.
...
onboarding-home = VoidReality에 어서오세요!
...
onboarding-automatic_mounting-description = VoidReality 트래커가 작동하려면, 실제 트래커의 장착 위치와 맞게 트래커의 착용 방향을 설정해야 해요.
...
onboarding-automatic_proportions-description = VoidReality 트래커가 작동하려면 뼈의 길이를 알아야 하는데, 이 짧은 보정을 통해 측정할 수 있어요.
...
onboarding-automatic_proportions-requirements-descriptionv2 =
    ...
    VR 헤드셋을 착용하세요.
    VR 헤드셋과 컨트롤러의 위치 정보가 VoidReality 서버에 실시간으로 표시되는지 확인하세요.
    VoidReality 서버에 연결된 트래커와 VR 헤드셋이 끊김이나 렉 없이 제대로 표시되는지 확인하세요. (이것은 SteamVR이 실행 중이며 VoidReality의 SteamVR 드라이버를 사용하여 VoidReality에 올바르게 연결되어 있다는 것을 의미합니다)
...
trackers_still_on-modal-description = 하나 이상의 트래커가 여전히 켜져 있는 것으로 보여요. VoidReality을 종료할까요?
trackers_still_on-modal-confirm = VoidReality 종료
...
status_system-StatusSteamVRDisconnected =
    { $type ->
        [steamvr_feeder] VoidReality 피더와 연결되지 않음
       *[other] VoidReality 드라이버가 SteamVR과 연결되지 않음
    }
...
firmware_tool-flashbtn_step-board_SLIMEVR = 트래커를 끄고, 케이스를 제거하고 (만약 있다면), 이 컴퓨터에 USB 케이블을 연결한 후 VoidReality 보드 버전에 따라 해당하는 작업을 수행해 주세요:
...
firmware_update-devices-description = 최신 버전의 VoidReality 펌웨어로 업데이트하려는 트래커들을 선택하십시오.
...
tray_or_exit_modal-radio-exit = VoidReality 종료하기
