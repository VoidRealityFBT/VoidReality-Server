...
websocket-connection_lost-desc = 看起来 VoidReality 服务器崩溃了。检查日志并重新启动程序
websocket-timedout = 无法连接到服务器
websocket-timedout-desc = 看起来 VoidReality 服务器崩溃或超时了。检查日志并重新启动程序
websocket-error-close = 退出 VoidReality
websocket-error-logs = 打开日志文件夹

## Update notification

version_update-title = 新版本可用：{ $version }
version_update-description = 点击“{ version_update-update }”将为您下载 VoidReality 安装程序。
...
tips-turn_on_tracker = 如果使用的是 VoidReality 官方的追踪器，请在将追踪器连接到电脑后再<b><em>打开追踪器的电源</em></b>！
...
board_type-SLIMEVR_DEV = VoidReality 开发板
...
board_type-GLOVE_IMU_SLIMEVR_DEV = VoidReality开发版IMU手套
board_type-GESTURES = 手势
board_type-ESP32S3_SUPERMINI = ESP32-S3 Supermini
board_type-GENERIC_NRF = nRF系列
board_type-SLIMEVR_BUTTERFLY_DEV = VoidReality蝴蝶 开发版
board_type-SLIMEVR_BUTTERFLY = VoidReality蝴蝶
...
tracker-settings-forget-description = 从 VoidReality 服务器中移除该追踪器，并在服务器重启前不再连接这一追踪器。追踪器的配置信息不会被清除。
...
settings-general-tracker_mechanics-save_mounting_reset-description =
    在VoidReality服务器关闭时保留追踪器自动佩戴重置结果。适用于一体式动捕服等
...
settings-interface-appearance-decorations-description = 这个选项开启后，将不会显示VoidReality的标题栏，而是显示使用系统原生标题栏。
...
settings-general-interface-connected_trackers_warning-description = 启用本选项后，每次当退出 VoidReality 时仍有追踪器连接着会显示通知，提醒你在使用完毕时关闭追踪器电源来节省电池电量。
...
settings-general-interface-use_tray-description = 关闭 VoidReality 窗口时，VoidReality 服务器将会隐藏至任务栏图标而不会直接退出，可以继续使用。
settings-general-interface-use_tray-label = 最小化至任务栏
settings-general-interface-discord_presence = 向Discord发送状态
settings-general-interface-discord_presence-description = 告诉你的Discord客户端你正在使用VoidReality，同时显示你正在使用的IMU追踪器数量。
...
settings-interface-behavior-error_tracking-description_v2 =
    ...
    <b>我们不会收集您的个人信息</b> ，例如您的 IP 地址或 WiFi 信息。VoidReality 重视您的隐私！
    
    为了提供最佳用户体验，我们会收集匿名错误报告、性能指标和操作系统信息。这有助于我们检测 VoidReality 的错误和问题。这些指标将通过 Sentry.io 收集。
...
settings-osc-router-network-description =
    ...
    这些端口可以与 VoidReality 服务器中使用的其他端口相同。
...
settings-osc-vmc-description =
    ...
    以发送 VoidReality 的骨骼数据，并接收来自其他应用程序的骨骼数据
...
settings-utils-advanced-open_data-description-v1 = 在文件管理器中打开VoidReality的配置文件夹，查看配置文件。
settings-utils-advanced-open_data-label = 打开文件夹
settings-utils-advanced-open_logs = 日志文件夹
settings-utils-advanced-open_logs-description = 在文件管理器中打开VoidReality的日志文件夹，查看VoidReality的日志文件。
...
onboarding-setup_warning =
    ...
    如果这是您第一次使用 VoidReality 则需要先进行设置。
...
onboarding-quiz-slimeset-official-sets = 官方VoidReality追踪器
onboarding-quiz-slimeset-thirdparty-sets = 第三方或DIY追踪器
onboarding-quiz-slimeset-answer-regular = VoidReality V1.0 和 V1.2
...
onboarding-quiz-usage-title = 将使用VoidReality进行何种应用？
onboarding-quiz-usage-description = 如果打算将VoidReality用至不同用途，可之后在设置中进行修改。
...
onboarding-quiz-mocap_preferences-desc = 设置使用VoidReality进行动作捕捉或虚拟偶像直播的方式
onboarding-quiz-mocap_preferences-playspace-title = 将在怎样的空间内使用？
onboarding-quiz-mocap_preferences-playspace-desc = 如果选择站立使用，VoidReality会试图追踪你的行走动作，而不是固定在原地。
...
onboarding-wifi_creds-description-v2 =
    大多数追踪器（例如官方的 VoidReality 追踪器）都通过 Wi-Fi 连接服务器。
...
onboarding-home = 欢迎来到 VoidReality
...
onboarding-connect_serial-error-modal-no_serial_device_found-desc =
    ...
    若仍无法工作：
     - 尝试其他的USB线
     - 尝试其他的USB接口
     - 尝试重新安装VoidReality服务端并在安装时勾选“USB驱动”
...
onboarding-automatic_mounting-description = 为了让 VoidReality 追踪器正常工作，我们需要为你的追踪器分配一个佩戴方向，以使其与你的物理追踪器佩戴方式对齐。
...
onboarding-automatic_proportions-description = 为了让 VoidReality 追踪器正常使用，我们需要知道你的骨头的长度。这个简短的校准将为你测量它们。
...
onboarding-automatic_proportions-requirements-descriptionv2 =
    ...
    你的追踪器和头戴设备都已经连接到 VoidReality 服务器并正常工作（没有卡顿、断联等问题）。
    你的头戴设备正在向 VoidReality 服务器回报位置信息（通常需要 SteamVR 正在运行且通过 VoidReality 的 SteamVR 驱动连接到 VoidReality）。
...
onboarding-user_height-description = 我们需要你的身高来计算躯干比例，以准确呈现你的动作。你可以让 VoidReality 自动计算身高，也可以手动输入。
...
trackers_still_on-modal-description =
    至少有一个追踪器的电源还开着。
    确定要退出 VoidReality 吗？
trackers_still_on-modal-confirm = 退出 VoidReality
...
status_system-StatusSteamVRDisconnected =
    { $type ->
        [steamvr_feeder] 尚未连接到 VoidReality Feeder App
       *[other] 尚未通过 VoidReality 驱动程序连接到 SteamVR
    }
status_system-StatusTrackerError = { $trackerName } 追踪器发生错误
status_system-StatusUnassignedHMD = VR头显应被分配为头部追踪器。
status_system-StatusPublicNetwork = 您当前的网络类型设置为“公开”。 这可能会导致VoidReality功能出现异常。<PublicFixLink>点击这里查看如何进行设置。</PublicFixLink>
...
firmware_tool-flashbtn_step-board_SLIMEVR = 关闭追踪器，拆下外壳（如果有的话），使用 USB 数据线连接到计算机，然后根据您的 VoidReality 电路板版本执行以下步骤之一：
...
firmware_update-devices-description = 请选择要更新到最新版本 VoidReality 固件的跟踪器
...
tray_or_exit_modal-description =
    你可以选择在按下关闭按钮时，是退出 VoidReality 服务器，还是仅将窗口最小化至任务栏图标。
    你也可以在设置-交互界面中修改这个选项
tray_or_exit_modal-radio-exit = 退出 VoidReality
...
unknown_device-modal-description =
    ...
    要将它连接到 VoidReality 吗？
unknown_device-modal-confirm = 是的！
unknown_device-modal-forget = 忽略它
# VRChat config warnings
vrc_config-page-title = VRChat设置警告
vrc_config-page-desc = 本页会显示当前VRChat设置的状态，并显示哪些设置与VoidReality存在冲突。为获得最好的体验，建议修复此处显示的所有警告后再进行使用。
...
tracking_checklist-status-incomplete = 使用 VoidReality 前的准备工作尚未完成！
tracking_checklist-status-partial = 你有 { $count } 个警告！
tracking_checklist-status-complete = 已经准备好使用 VoidReality！
...
tracking_checklist-VRCHAT_SETTINGS-desc = VRChat 的设置有问题！这会影响到在 VRChat 中使用 VoidReality 的体验。
